...
## Como funciona a integração entre sistemas do Hashdata?
...
projetar serviços de rede.

A integração via API REST permite que outros sistemas acessem os dados armazenados no Hashdata. A comunicação entre os
...
O acesso aos dados é feito por meio dos *end-point*. As URI's de cada recurso é apresentadas a seguir:
...
Considerando que o único formulário configurado para a integração é o "Formulário de Dados Pessoais", ao executar a
...
#### **USUÁRIOS**
Para listar todos os usuários de uma determinada área de trabalho, utiliza o seguinte *end-point*:

* https://api2.hashdata.app/v1/export/users?token={TOKEN}

Por padrão, esse *end-point* retorna os dados no formato JSON.
Ao executar a requisição informando apenas o *token* de autenticação, obtém-se uma resposta como a do exemplo a seguir:
...
É possível receber os dados dos usuários no formato CSV (*Comma-Separated Values*). Para isso, basta informar mais dois
...
#### **RESPOSTAS**
Para obter as respostas de um determinado formulário, é necessário informar, além do *token* de autenticação, o `formID`
...
Use os parâmetros `instantStart` e `instantEnd` para paginar a resposta ou para obter os dados progressivamente (conforme inseridos no sistema).

:::success Formatos aceitos
Os parâmetros `instantStart` e `instantEnd` aceitam qualquer formato de `Date` do Javascript, por exemplo, data no
...
https://api2.hashdata.app/v1/export/forms/66f172c1dbb522a3729ab9a1/responses?token={TOKEN}&instantStart=2025-01-01T15:00:00.000Z

https://api2.hashdata.app/v1/export/forms/66f172c1dbb522a3729ab9a1/responses?token=jfRejrLXrQG7R3rO6kM0uCdGaq7zt_Y_f9uhw5Y1&instantStart=2025-01-01T15:00:00.000

:::

````mdx-code-block
<details>
    <summary>**🔒 Envio do Token de Autenticação em uma API REST: URI vs. Cabeçalho**</summary>
    <article>
        Em uma API REST, o **token de autenticação** pode ser enviado de duas formas para validar e autorizar
o acesso a recursos protegidos:

        1. **Envio pela URI (*Query Parameter*)**
        2. **Envio pelo Cabeçalho HTTP (HD-API-TOKEN)**
        ...
        <h3> **2. Envio do Token pelo Cabeçalho HTTP (HD-API-TOKEN)** </h3>
...
### 2. Integração com Hashdata Advanced Reports
...
painéis quanto os relatórios são fáceis de criar e não requerem conhecimento especializado.

<h4></h4> 3. Formulários exportados
Após definir quais integrações serão utilizadas, é necessário informar quais formulários devem ser integrados ao
...
### 3. Configuração da integração dos Formulários
